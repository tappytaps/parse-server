...
RestWrite.prototype.runBeforeTrigger = function () {
  ...
  var updatedObject = this.buildUpdatedObject(extraData);
  if (this.query && this.query.objectId) {
...
RestWrite.prototype.handleAuthData = function (authData) {
  ...
  var results = void 0;
  return this.findUsersWithAuthData(authData).then(function (r) {
    results = r;
    if (results.length > 1) {
    ...
    if (results.length > 0) {
      ...
      var hasMutatedAuthData = Object.keys(mutatedAuthData).length !== 0;
      var userId = void 0;
      if (_this6.query && _this6.query.objectId) {
        userId = _this6.query.objectId;
      } else if (_this6.auth && _this6.auth.user && _this6.auth.user.id) {
        userId = _this6.auth.user.id;
      }
      if (!userId || userId === userResult.objectId) {
        ...
        return _this6.handleAuthDataValidation(mutatedAuthData).then(function () {
    ...
    return _this6.handleAuthDataValidation(authData);
  });
};
...
RestWrite.prototype.transformUser = function () {
  ...
  if (!this.auth.isMaster && "emailVerified" in this.data) {
    var error = 'Clients aren\'t allowed to manually update email verification.';
...
RestWrite.prototype._validateEmail = function () {
  ...
  return this.config.database.find(this.className, { email: this.data.email, objectId: { '$ne': this.objectId() } }, { limit: 1 }).then(function (results) {
    ...
    if (!_this8.data.authData || !Object.keys(_this8.data.authData).length || Object.keys(_this8.data.authData).length === 1 && Object.keys(_this8.data.authData)[0] === 'anonymous') {
  ...
  promise = promise.then(function () {
    ...
    if (!idMatch) {
      if (!deviceTokenMatches.length) {
        ...
        _this13.config.database.destroy('_Installation', delQuery).catch(function (err) {
      ...
      if (deviceTokenMatches.length == 1 && !deviceTokenMatches[0]['installationId']) {
        ...
        if (_this13.data.deviceToken && idMatch.deviceToken != _this13.data.deviceToken) {
          ...
          _this13.config.database.destroy('_Installation', _delQuery2).catch(function (err) {
            if (err.code == Parse.Error.OBJECT_NOT_FOUND) {
              // no deletions were made. Can be ignored.
              return;
            }
...
RestWrite.prototype.buildUpdatedObject = function (extraData) {
  ...
  Object.keys(this.data).reduce(function (data, key) {
    if (key.indexOf(".") > 0) {
      ...
      if ((typeof parentVal === 'undefined' ? 'undefined' : _typeof(parentVal)) !== 'object') {
        parentVal = {};
      }
